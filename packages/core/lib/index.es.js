...
import { Plugin as be, PluginKey as De, EditorState as nt } from "@milkdown/prose/state";
...
const dt = new De("MILKDOWN_STATE_TRACKER");
function ht(e) {
  const t = Ze(
    st,
    xe,
...
const pt = new De("MILKDOWN_VIEW_CLEAR"), Be = (e) => (e.inject(me, document.body).inject(L, {}).inject(ce, {}).inject(we, null).inject(ge, {}).inject(ae, [J]).record(oe), async () => {
  await e.wait(M);
